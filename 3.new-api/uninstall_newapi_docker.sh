...
#
# ⚠️  警告：
#   - 此操作将删除所有 New-API 数据（包括数据库）
#   - 删除前会提示备份选项
#   - 不影响 LiteLLM、CliproxyAPI 等其他服务
#
# 使用方法：
#   chmod +x uninstall_newapi_docker.sh
...
clear
echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${RED}   New-API Docker 卸载程序${NC}"
echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
read -p "是否继续卸载 New-API？(yes/NO): " CONFIRM
...
if [[ ! "$BACKUP_CHOICE" =~ ^[Nn]$ ]]; then
    ...
    if [ -f "$SERVICE_DIR/newapi_info.txt" ]; then
        cp "$SERVICE_DIR/newapi_info.txt" "$BACKUP_DIR/"
        log_success "已备份 newapi_info.txt"
    fi
    ...
    if $COMPOSE_CMD ps | grep -q "Up"; then
        ...
        elif $COMPOSE_CMD ps | grep -q "mysql"; then
            DB_PASSWORD=$(grep MYSQL_PASSWORD "$SERVICE_DIR/docker-compose.yml" | head -1 | sed 's/.*: *//')
...
if [ -f docker-compose.yml ]; then
    ...
    if [[ "$DELETE_VOLUMES" =~ ^[Yy]$ ]]; then
        ...
        log_info "数据卷可手动删除: docker volume ls | grep newapi"
    fi
else
...
if [[ ! "$DELETE_VOLUMES" =~ ^[Yy]$ ]]; then
    ...
    echo -e "${YELLOW}[注意]${NC}"
    echo -e "数据卷已保留，可手动清理："
    echo -e "  docker volume ls | grep newapi"
    echo -e "  docker volume rm newapi_postgres_data newapi_redis_data"
fi
